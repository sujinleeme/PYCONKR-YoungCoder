...
프로그래밍에 대해 이야기 할 때 우리는 무엇을 이야기 하나요? 모든 프로그램은 당신이 풀고 싶은 문제로 시작됩니다. 이 문제에 대한 해결책을 알고리즘이라고 합니다 

마지막으로, 이 해결책은 컴퓨터가 이해할 수있는 Python과 같은 프로그래밍 언어로 번역됩니다. 컴퓨터에서 실행되는 코드 패키지를 프로그램이라고 합니다

그렇다면 컴퓨터는 어떻습니까? 이미 여러분들은 컴퓨터가 무엇이고, 무엇을 할 수 있을지 바로 생각할 수 있지요? 
컴퓨터는 일종의 기계이고 기본적으로 두 가지 간단한 것으로 나뉩니다.

하나는 우리가 데이터라고 부르는 정보를 저장합니다.
그리고 컴퓨터가 이해하는 프로그램을 사용해 그 데이터로 일을 처리합니다.

컴퓨터는 다양한 형태가 있어요. 노트북이나 PC 일 수도 있고 폰도 컴퓨터 일 수도 있다는 거 알고 있었나요? 
