...
프로그래밍에 관해 이야기 할 때 우리는 무엇을 의미합니까? 음, 모든 프로그램은 당신이 풀고 싶은 문제로 시작됩니다. 이 문제에 대한 해결책을 알고리즘이라고합니다 (이에 대해서는 몇 분 후에 더 자세히 설명 할 것입니다). 

마지막으로,이 솔루션은 컴퓨터가 이해할 수있는 Python과 같은 프로그래밍 언어로 번역됩니다. 컴퓨터에서 실행되는 코드 패키지를 프로그램이라고합니다
